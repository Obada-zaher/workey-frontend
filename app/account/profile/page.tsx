...
export default async function ProfilePage() {
  ...
  if (!profile) { const message = loadError instanceof AuthBackendError && loadError.status === 403 ? "This account area is only available to job seekers." : "Your profile information is temporarily unavailable. Please try again shortly."; return <div className="max-w-3xl"><h1 className="type-heading-1 text-text-primary">Your profile</h1><p className="radius-medium mt-5 border border-border-danger bg-danger-surface p-4 type-body text-danger" role="alert">{message}</p></div>; }
  return <ProfileEditor initialProfile={profile} user={user} />;
}
