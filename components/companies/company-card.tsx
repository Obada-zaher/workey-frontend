...
export function CompanyCard({ company, variant = "default" }: { company: FeaturedCompany; variant?: "default" | "mini" }) {
  ...
  return <article className={`company-card ui-card h-full${variant === "mini" ? " company-card--mini" : ""}`}><div aria-hidden="true" className="company-card__cover">{coverUrl && !coverFailed ? <img alt="" loading="lazy" onError={() => setCoverFailed(true)} src={coverUrl} /> : null}</div><div className="company-card__logo"><CompanyIdentity logoUrl={company.logo_url} name={company.name} /></div><div className="company-card__content"><h3 className="type-heading-3 text-text-primary">{company.name}</h3>{company.industry ? <p className="type-body-small text-text-secondary">{company.industry}</p> : null}{company.location && variant === "default" ? <p className="type-body-small text-text-muted">{company.location}</p> : null}<Badge variant="neutral">{company.open_jobs_count} open {company.open_jobs_count === 1 ? "job" : "jobs"}</Badge></div></article>;
}
