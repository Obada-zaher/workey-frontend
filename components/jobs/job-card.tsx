...
export function JobCard({ job, recommendation, variant = "default" }: { job: HomeJob | Job; recommendation?: RecommendationDetails; variant?: JobCardVariant }) {
  ...
  return <article className={`job-card ui-card ui-card--interactive job-card--${variant} ${hasCover ? "job-card--has-cover" : "job-card--no-cover"}`}>
    {hasCover && coverUrl ? <div aria-hidden="true" className="job-card__media"><img alt="" loading="lazy" onError={() => setFailedCoverUrl(coverUrl)} src={coverUrl} /><span className="job-card__media-logo"><CompanyIdentity context="media" logoUrl={logoUrl} name={companyName} size={variant === "explore" ? "medium" : "small"} /></span></div> : null}
    <div className="job-card__foreground">
      <header className="job-card__identity">{!hasCover ? <CompanyIdentity logoUrl={logoUrl} name={companyName} size={variant === "explore" ? "medium" : "small"} /> : null}<div className="job-card__title-group"><p className="job-card__company">{companyName}</p><h3 className="job-card__title"><Link aria-label={`View ${job.title} at ${companyName}`} href={detailHref}>{job.title}</Link></h3></div>{score !== null ? <Badge variant="primary">{score}% Match</Badge> : null}</header>
